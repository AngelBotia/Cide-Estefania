...
export default class RecoveryController extends Controller {
  ...
  onSubmit(event) {
    ...
    const currentLocalLogin = JSON.parse(
      localStorage.getItem(Constants.USER_STORAGE),
    );
    const indexToEdit = currentLocalLogin.findIndex(
      (item) => item.user == formData.user,
    );
    currentLocalLogin[indexToEdit].password = formData.password;
    localStorage.setItem(
      Constants.USER_STORAGE,
      JSON.stringify(currentLocalLogin),
    );
    window.alert(
      `El cambio de contraseña al usuario:${formData.user} ha sido un exito ✅`,
    );
    this.clearForm(form);
  }
