  ...
  filterStudentList(filter) {
    const result = this.studenlist.filter((x) => {
      return (
        ...
        x.apellidos.toLocaleLowerCase().includes(filter) ||
        x.direccion.toLocaleLowerCase().includes(filter) ||
        x.dni.toLocaleLowerCase().includes(filter) ||
        x.direccion.toLocaleLowerCase().includes(filter) ||
        x.fechaNacimiento.toLocaleLowerCase().includes(filter)
      );
    });
